...
* - Statement 에 SQL을 미리 준비 시켜 놓고
*  ? (위치 홀더, placeholder) 자리에  알맞은 값을 세팅 후 수핼하는 객체
*
* - Statement의 자식 인터페이스
* - 장점
*  1) SQL Injection 방지
    ...
    public static void main(String[] args) {
        ...
        try{
           ...
           // Placeholder(?)를 포함한 SQL
            ...
            // prepareStatement  객체가 만들어지는 시점에
            // 이미 SQL이 전달되어 있다.
            ...
            // SQL 을 수핼만 시키면 되기 때문이다
            rset = pstmt.executeQuery();

            // 조회 결과가 있다면
            if(rset.next()){
                System.out.println(rset.getInt("category_code") +
                        " / "+
                        rset.getInt("menu_name"));
            }
