package com.work.section03.sqlinjection;
...
import static com.work.common.JDBCTemplate.getConnection;
...
public class Application1 {
  public static void main(String[] args) {
    ...
    try {
      ...
      String sql = "SELECT * FROM tbl_menu where menu_code = " + menuCode + " and menu_name = '"+menuName+"'" ;  //DQL Data Query Language(질의 언어)
      //excuteQuery() : SELECT 수행 후 ResultSet 반환
      //ResultSet  : Select 결과를 다루는 객체 (1행 씩)
      rset = stmt.executeQuery(sql);

      while(rset.next()){ // 다음 행이 있다면 이동하고 true(반복)
//        //한행에서 특정 컬럼 값을 자바 자료형에 맞춰 얻어오기
